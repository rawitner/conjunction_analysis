...
```{r }
top50 %>% dplyr::select(c("noradId", "cluster_new", "totalRisk", "num conjs","closest approach (m)",
                          "avg num op sats","med num op sats",
                          "avg persist of conj alt",
                          "med persist of conj alt", "num encounters <1 km")) %>%
...
top50 %>% dplyr::select(c("noradId", "cluster_new", "totalRisk","apogee",
                          "perigee","launch","inclination","mass","type","country")) %>%
  rename(cluster=cluster_new,
         `total risk`=totalRisk) %>%
  DT::datatable(rownames=F, extensions = 'Buttons',
                options = list( lengthMenu = c(10, 20, 30, 40, 50),
                                buttons = 'csv',
                                dom = 'Bfrtip')) %>%
  formatRound(columns = c("total risk"), digits=1) %>%
