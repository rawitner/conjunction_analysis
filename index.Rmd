...
all_conjs = all_conjs %>%
  mutate(risk = (combinedMass + persistence + SD) / Range,
         ...
         persistence_s = rescale(persistence, to=c(0,20)),
         SD_s = rescale(SD, to=c(0,100)),
         Range_s = if_else(Range >= 1, 10^(Range), 10), #rescale(Range, to=c(1e-4, 100)),
         risk_s = (combinedMass_s + persistence_s + SD_s) / (Range_s)) #%>%

# subset = all_conjs %>% 
#   dplyr::select(c(clusterLab, combinedMass, combinedMass_s, persistence, persistence_s,SD, SD_s, Range, Range_s, risk, risk_s ))
...
#mcma_objs = mcma_objs %>% mutate(persistence_new = exp(intercept + slope * avgAlt))

#x = c("cc615","cc775","cc850","cc975","cc1200","cc1500","cleo","CHIGH")
#mcma_objs %>% group_by(cluster_new) %>%
#  summarise(avgPersistence = mean(persistence_new, na.rm = T)) %>%
#  slice(match(x, cluster_new))

#all_conjs %>% filter(Altitude < 2000) %>%
#  ggplot(aes(x=Altitude, y=numOpSats)) +
#  geom_line()
```
...
![SMC derelicts graphic](misc_files/SMCderelicts.png)

The weights are selected so that combined mass and persistence both vary on a scale from 0 to 100 and number of operational satellites varies on a scale from 0 to 20. 
