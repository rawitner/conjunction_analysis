...
mycolors = c(brewer.pal(name="Set1", n = 8), brewer.pal(name="Set2", n = 3))
mycolors[6] = "#000000"
...
all_conjs %>%
  ...
  ggplot(aes(x=Range, y = cumnum, color=clusterLab, linetype=clusterLab)) + 
  geom_line() +
  theme_light() +
...
alt_bins = mcma_objs %>% 
  ...
  mutate(numSats = replace_na(numSats, 0)) %>% 
  mutate(spatDens = numSats / volume * (10^12))

mcma_objs = mcma_objs %>% 
  mutate(altitude = roundDown((as.numeric(apogee) + as.numeric(perigee))/2)) %>% 
  left_join(select(alt_bins, c(altitude, spatDens)), by="altitude")
...
df_data = readxl::read_xlsx("misc_files/DF.xlsx", skip = 2, 
                            col_names = c("altitude","atmos_dens", "DF"))
...
df_data$predicted = exp(intercept + slope * df_data$altitude)
df_data$predicted_df = df_data$predicted / ad_at_500
...
cluster_summary = all_conjs_expanded %>% 
  ...
  mutate(diam_m = .005*avgMass,
         AC = (pi*(diam_m/1000)^2)/4, #2*(.005*avgMass), #  [(Pi)*d^2]/4
         lambda = (numSats / volume) * AC * velocity,
...
```{r getSMC}
cols_ordered = c("noradId", "cluster_new","SMC_risk", "E5", "E1", "spatDens", "df", "AR", "MF","velocity","SD","AC","CR",
...
SMC_out = all_conjs_expanded %>% 
  ...
  mutate(SMC_risk = (CR * AR * E5/25 * 5 * E1) * (MF * df * spatDens)) %>%
...
SMC_out %>%
  ...
  formatRound(columns = c("SMC_risk","spatDens","df","velocity","SD","AC","CR", "MF"), digits=2) %>%
  formatDate(columns = "launch", method = "toDateString", params = list('en-US', list(year = 'numeric', month = 'numeric', day='numeric')
      ))

```

Percent of risk accounted for by each cluster among all objects:
