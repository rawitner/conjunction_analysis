...
- AR = cross-sectional area ($\frac{area~of~object}{10}$)
- E1 = number of encounters < 1km per year
- E.5 = number of encounters < 0.5km per year

Consequence:

- MF = mass of object (proportional to amount of debris produced)
- DF = $\frac{atmospheric~density ~(AD) ~at~ 500km}{AD ~at ~object's ~avg. ~altitude}$
- SF = spatial density of operational satellites 

Then for each object, the risk is calculated as:
$$Risk = [CR * AR * {E1 * (2 * E.5)}] * [MF * DF * SF]$$

```{r}
all_conjs_expanded %>% 
  group_by(noradId) %>%
  summarise(numtot = length(Range),
            num1 = length(Range[Range<=1]),
            num.5 = length(Range[Range<=0.5])) %>%
  right_join(mcma_objs, by="noradId") %>% 
  mutate(numtot = replace_na(numtot, 0),
         num1 = replace_na(num1, 0),
         num.5 = replace_na(num.5, 0)) #%>%
  #View()
