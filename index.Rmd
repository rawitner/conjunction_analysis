...
all_conjs = all_conjs %>%
  mutate(risk = (combinedMass + persistence + numOpSats) / Range,
         ...
         numOpSats_s = rescale(numOpSats, to=c(0,20)),
         Range_s = 10^(Range), #rescale(Range, to=c(1e-4, 100)),
         risk_s = (combinedMass_s + persistence_s + numOpSats_s) / (Range_s)) #%>%

#subset = all_conjs %>% 
#  dplyr::select(c(combinedMass, combinedMass_s, persistence, #persistence_s,numOpSats, numOpSats_s, Range, Range_s, risk, risk_s ))
...
mcma_objs = mcma_objs %>% 
  mutate(persistence_new = predict(lw1, avgAlt))
